...
import { Major } from '../../hooks/useAdminData';
import { getCORSFriendlyUrl, createImage, getCroppedImg } from './CreateMajorForm';

interface CreateCourseFormProps {
  onClose: () => void;
  onSave: (title: string, majorId: string, description: string, imageUrl: string) => Promise<any>;
  initialData?: {
    id?: string;
    title: string;
    major_id: string;
...
export default function CreateCourseForm({ 
  ...
  const [majorId, setMajorId] = useState(initialData?.major_id || defaultMajorId || '');
  ...
  const [aspect, setAspect] = useState<number>(16 / 9); // default course layout ratio
  ...
  const applyCrop = async () => {
    ...
    try {
      ...
      const croppedFile = new File([croppedBlob], `cropped-course-${Date.now()}.jpg`, {
        type: 'image/jpeg',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !majorId || saving) return;

    setSaving(true);
    setErrorLog(null);
    try {
      await onSave(title, majorId, description, imageUrl);
      onClose();
    } catch (err: any) {
      console.error(err);
      setErrorLog(err.message || 'Failed to submit attributes.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-md flex items-center justify-center z-[100] p-4 animate-fade-in" id="course_form_modal">
      ...
      {/* --- CROP MODAL OVERLAY --- */}
      {isCropOpen && srcImage && (
        <div className="fixed inset-0 bg-slate-950/95 z-[150] flex flex-col justify-between p-6 md:p-8 animate-fade-in" id="course_crop_overlay">
          <div className="max-w-4xl mx-auto w-full flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-white tracking-tight">Crop Course Banner</h3>
              <p className="text-[10px] font-black text-indigo-405 uppercase tracking-wider">Configure precise alignment and coverage</p>
            </div>
